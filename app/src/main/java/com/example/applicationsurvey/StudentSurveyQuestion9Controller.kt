...
class StudentSurveyQuestion9Controller : AppCompatActivity() {
    // Database helper instance
    val db: DataBaseHelper = DataBaseHelper(this)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_survey_question9)
    ...
    // Set the ninth question based on the survey ID
    fun setQuestion(surveyID:String){
        val questionList :ArrayList<Question> = db.getQuestions(surveyID)
        val question :String = questionList.get(8).QuestionText
        val question1 = findViewById<TextView>(R.id.txtTakeQuestion9)
        question1.text = "9. $question"
    }

    // Proceed to the next question
    fun gotoNextQuestion(view: View){
        val radioButton1 = findViewById<RadioButton>(R.id.rdq9a1)
        val radioButton2 = findViewById<RadioButton>(R.id.rdq9a2)
        val radioButton3 = findViewById<RadioButton>(R.id.rdq9a3)
        val radioButton4 = findViewById<RadioButton>(R.id.rdq9a4)
        val radioButton5 = findViewById<RadioButton>(R.id.rdq9a5)

        val surveyId = intent.getStringExtra("PublishSurvey").toString()
        val questionList :ArrayList<Question> = db.getQuestions(surveyId)
        val questionId :String = questionList.get(8).Id.toString()
        ...
        if(radioButton1.isChecked || radioButton2.isChecked || radioButton3.isChecked || radioButton4.isChecked || radioButton5.isChecked) {
            val radioGroup = findViewById<RadioGroup>(R.id.radioA9)
            ...
            val intent = Intent(this, StudentSurveyQuestion10Controller::class.java)
