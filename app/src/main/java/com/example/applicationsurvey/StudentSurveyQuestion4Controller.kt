...
class StudentSurveyQuestion4Controller : AppCompatActivity() {
    ...
    // Function to set the fourth question based on the survey ID
    private fun setQuestion(surveyId: String) {
        val questionList: ArrayList<Question> = dbHelper.getQuestions(surveyId)
        val question: String = questionList[3].QuestionText
        val questionTextView = findViewById<TextView>(R.id.txtTakeQuestion4)
        questionTextView.text = "4. $question"
    }
    ...
    fun gotoNextQuestion(view: View) {
        ...
        val surveyId = intent.getStringExtra("PublishSurvey").toString()
        val questionList: ArrayList<Question> = dbHelper.getQuestions(surveyId)
        val questionId: String = questionList[3].Id.toString()
